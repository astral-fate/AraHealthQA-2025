# -*- coding: utf-8 -*-
"""
Final Consolidated EDA for MCQ Model Error Analysis
...
fitb_model_data = [
    {
        ...
        'csv_path': '/content/drive/MyDrive/AraHealthQA/t2t1/final_result/predictions_fitb_choices_MedGemma.csv',
      
        'time_minutes': 176 + 4/60
    },
    {
        'model_name': 'Colosseum (Fallback)',
        'csv_path': '/content/drive/MyDrive/AraHealthQA/t2t1/final_result/predictions_fitb_choices_Colosseum_with_Fallback.csv',
     
        'time_minutes': 14 + 43/60
    },
    {
        'model_name': 'Llama3 70B (Zero-Shot)',
        'csv_path': '/content/drive/MyDrive/AraHealthQA/predictions_fitb_choices.csv',
   
        'time_minutes': None
    },
    {
        'model_name': 'Llama3 70B (CoT)',
        'csv_path': '/content/drive/MyDrive/AraHealthQA/t2t1/final_result/predictions_fitb_choices_CoT.csv',
    
        'time_minutes': 27 + 20/60
    },
    {
        'model_name': 'Palmyra-Med (Fallback)',
        'csv_path': '/content/drive/MyDrive/AraHealthQA/t2t1/final_result/predictions_fitb_choices_Palmyra-Med_with_Fallback.csv',
      
        'time_minutes': 13 + 57/60
    },
    {
        'model_name': 'BiMediX2 (vLLM)',
        'csv_path': '/content/drive/MyDrive/AraHealthQA/t2t1/final_result/predictions_mcq_choices_BiMediX2.csv',
        
        'time_minutes': 10 + 4/60
    },
    {
        'model_name': 'Mixtral (Fallback)',
        'csv_path': '/content/drive/MyDrive/AraHealthQA/t2t1/final_result/predictions_fitb_choices_Mixtral_with_Fallback.csv',
    
        'time_minutes': 15 + 54/60
    },
    {
        'model_name': 'OpenBioLLM (Local)',
        'csv_path': '/content/drive/MyDrive/AraHealthQA/t2t1/data/predictions_fitb_choices_OpenBioLLM.csv',
     
        'time_minutes': 10 + 47/60
    },
    {
        'model_name': 'BioMistral (Local)',
        'csv_path': '/content/drive/MyDrive/AraHealthQA/t2t1/final_result/predictions_fitb_choices_BioMistral_SLERP.csv',
     
        'time_minutes': 47 + 46/60
    },
    {
        'model_name': 'DeepSeek 70B (Zero-Shot)',
        'csv_path': '/content/drive/MyDrive/AraHealthQA/predictions_fitb_choices_deepseek.csv',
      
        'time_minutes': 14.0
    },
    {
        'model_name': 'Gemini',
        'csv_path': '/content/drive/MyDrive/AraHealthQA/fitb/fitb_v2.csv',
      
        'time_minutes': 4.0
    }
]
models_df_initial = pd.DataFrame(fitb_model_data)

# Load original ground truth data
try:
    original_data_path = '/content/drive/MyDrive/AraHealthQA/t2t1/data/fill-in-the-blank-choices.csv'
...
original_df['Answer_Letter'] = original_df['Answer - Arabic'].apply(extract_answer_letter)
...
if all_errors:
    ...
    print("\n" + "="*50)
    print("📊 Final EDA for MCQ Task (Calculated)")
    print("="*50)
    ...
    if not pivot_df.empty:
        error_pivot_table = pivot_df.pivot_table(index='Category', columns='model_name', values='Question - Arabic', aggfunc='count', fill_value=0)
...
if all_errors:
    ...
    plt.title('Error Distribution: Top 5 Models vs. Top 5 Categories (FITB Task)', fontsize=16, weight='bold', pad=20)
    ...
    ax1.set_title('Model Errors per Medical Category (FITB Task)', fontsize=18, weight='bold')
    ...
    ax2.text(ax2.get_xlim()[0], ax2.get_ylim()[1], ' High Accuracy, Fast ', fontsize=12, va='top', ha='left', backgroundcolor='w', color='green')
    ax2.text(ax2.get_xlim()[1], ax2.get_ylim()[0], ' Low Accuracy, Slow ', fontsize=12, va='bottom', ha='right', backgroundcolor='w', color='red')
    ax2.text(ax2.get_xlim()[0], ax2.get_ylim()[0], ' Low Accuracy, Fast ', fontsize=12, va='bottom', ha='left', backgroundcolor='w', color='blue')

    # Set titles and labels
    ax2.set_title('Quadrant Analysis: Model Accuracy vs. Execution Time (FITB Task)', fontsize=16, weight='bold', pad=20)
