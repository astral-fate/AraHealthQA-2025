...
import pandas as pd
import openai
import groq # Added for fallback
import time
...
OUTPUT_CSV = '/content/drive/MyDrive/AraHealthQA/t2t1/final_result/predictions_fitb_choices_Mixtral_with_Fallback.csv'
...
# --- NEW: Fallback Function using Groq Llama 3 ---
...
# --- Primary Function to Generate Answers (Modified to use its own prompt) ---
def generate_answer_primary(question):
    ...
    max_retries = 2 # Reduced retries, as we have a fallback
    retry_delay = 10
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                return "API_ERROR" # Return error to trigger fallback
    return "FAILED_ATTEMPTS" # Return error to trigger fallback


# --- Function to Evaluate MCQ Accuracy (Modified to include fallback errors) ---
def evaluate_mcq_accuracy(predictions, ground_truths):
    print("\n" + "="*50)
    print("🚀 Starting Evaluation...")
    print("="*50)

    # Add fallback error codes to the list of invalid predictions
...
# --- Main Execution (MODIFIED with Fallback Logic) ---
def main():
    try:
    ...
    if os.path.exists(OUTPUT_CSV):
        ...
        print(f"'{OUTPUT_CSV}' not found. Starting prediction process with Fallback System...")
        predictions = []
        ...
        for index, row in df.iterrows():
            ...
            print(f"Processing question {index + 1}/{total_questions} with Mixtral (Primary)...")
            
            # --- FALLBACK LOGIC ---
            answer_letter = generate_answer_primary(question)
            
            # If the primary model fails, trigger the fallback
